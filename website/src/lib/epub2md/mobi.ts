...
export default class Mobi {
  ...
  parse() {
    ...
    header = buffer.slice(
      pdbHeader.records[0].position,
      pdbHeader.records[1].position,
    )
    ...
    if (mobiHeader.headerLength >= 0xe4) {
      flags =
        ((_ref6 = new Packer('H').unpack_from(header, 0xf2)),
        (mobiHeader.flags = _ref6[0]),
        _ref6)
      multibyte = flags & 1
      while (flags > 1) {
        trailers += 1
